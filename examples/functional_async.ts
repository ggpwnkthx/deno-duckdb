...
const metadata = {
  statement_type: await result_statement_type(result),
  result_type: await result_return_type(result),
  columns: await Promise.all(Array.from({ length: Number( column_count(result)) }, async (_, i) => ({
    name: await column_name(result, i),
    type: await column_type(result, i)
  })))
}
